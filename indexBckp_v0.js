...
const sqlite3 = require('sqlite3').verbose();
    ...
    // take Home path -- 2pt
    // form maestro script -- 5pt (look ups from the file above)
    // run maestro script -- 1pt 
    // snap&plot path (if clickables exist) -- 2pt (code exists)
    // GPT: -- 7pt (too unknown)
        // 1. analyse plotted
        // 2. create/update map
        // 3. has this screen been seen before? 
    // GPT: -- 1pt (same as above)
        // 1. what to click?

    // form paths, add to PathList -- 2pt 
    // pop path if PathList not empty or finish -- 1pt
...
async function askRecommendationClickAllRecommendedElements(path_to_level, db) {
    ...
    try {
        await runPython(["clickAndExtract-AI.py", path_to_level]); // Use await here as well
        console.log("Completed clickAndExtract-AI.py with ", path_to_level)
    } catch (error) {
    ...
    // use PYTHON pHash - perceptual hash to find out if the image has been received before.
    ...
    console.log("Calling pHash-Test.py with ", list_of_pHashes, " \n and ", path_to_plotted_screenshot)
    try {
        ...
        console.error(`Error running pHash-Test.py:`, error);
    }
    ...
    // ask gpt to recommend elements to click
    let gpt_response = ""
    try {
    ...
    if (path_to_level === "/0") {
        try {
            ...
            // Save to the file
            await writeJsonToFile(current_json); // Assuming you want to write to the same file
        } catch (error) {
        ...
        try {
            ...
            // Save to the file
            await writeJsonToFile(current_map); // Assuming you want to write to the same file
        } catch (error) {
            console.error('Failed to process JSON:', error);
        }
    }  
...
async function writeJsonToFile(json, mapPath = gpt_map_file) {
    try {
